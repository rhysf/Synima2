...
main: {
	...
	my $n_matches = scalar(@match_pairs);
	print STDERR "Parsed $n_matches matches from $$dag_config{'inputFile'}\n" if($opt_v eq 'y');
	die "No matches parsed from $$dag_config{'inputFile'}\n" if $n_matches == 0;
	
	if($opt_v eq 'y') { warn "Saving gene features...\n"; }
	my @gene_features = &parse_gff_files($organism_to_gff_file);

	#print Dumper(@gene_features);
	#die "end here\n";
	...
	my $n_keys_matches = scalar(keys(%orgMolPair_to_matches));
	print STDERR "Parsed $n_keys_matches matches before noise filter\n" if($opt_v eq 'y');
	...
	$n_keys_matches = scalar(keys(%orgMolPair_to_matches));
	print STDERR "Parsed $n_keys_matches matches after noise filter\n" if($opt_v eq 'y');
...
sub run_DAG_chainer {
	...
	my $tmpFile = ".$$.tmpOut";
	my $cmd = "$dagchainer_program -G $$dag_config{'GAP_LENGTH'} -O $$dag_config{'GAP_OPEN_PENALTY'} -E $$dag_config{'GAP_EXTENSION_PENALTY'} -S $$dag_config{'MIN_ALIGNMENT_SCORE'} -D $$dag_config{'MAX_DIST_BETWEEN_MATCHES'} -F $filename $reverseOrientFlag > $tmpFile";
	warn "run_DAG_chainer: $cmd \n" if($opt_v eq 'y');
	#print "CMD: $cmd \n";
	my $ret = system $cmd;
	die "ERROR, couldn't run command\n\n$cmd\n(ret: $ret)\n\nMay need to recompile $dagchainer_program\n\n" if($ret);
...
sub save_dagchainer_config {
	my $conf = $_[0];
	die "save_dagchainer_config: Cannot find $conf. Rerun Synima -s dagchainer\n" if(! -e $conf);

	warn "save_dagchainer_config: $opt_c\n";
